...
2.5. Establish the discovery channel BEFORE searching (do not assume). The Claude-in-Chrome tools may be DEFERRED in this run — if you don't see `mcp__Claude_in_Chrome__list_connected_browsers`, load the Claude-in-Chrome tools first (tool search), then call it. If a browser is connected, use Chrome (tag source "chrome"); verify LinkedIn isn't showing a login wall. Only if no browser is connected (or LinkedIn is logged out) fall back to WebSearch `site:linkedin.com/in` (tag source "websearch"). Never fall back without having actually probed `list_connected_browsers`. State the channel and the reason in the summary.
3. Search for up to config.dailyTarget NEW contacts (hiring managers, recruiters, peers, alumni) matching the targets on the channel chosen in 2.5, EXCLUDING anyone whose normalized linkedinUrl is already in the queue (any status).
